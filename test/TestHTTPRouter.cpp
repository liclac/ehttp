...
#define REGISTER_ERROR_HANDLER(_code, _var, _text) \
	bool _var = false; \
	rtr.onError(_code, [&](std::shared_ptr<HTTPRequest> req, std::shared_ptr<HTTPResponse> res) { \
...
#define REGISTER_HANDLER(_method, _path, _var, _code, _text) \
	bool _var = false; \
	rtr.on(_method, _path, [&](std::shared_ptr<HTTPRequest> req, std::shared_ptr<HTTPResponse> res) { \
		res->begin(_code)->header("Content-Type", "text/plain")->write(_text)->end(); \
		_var = true; \
	})

#define ROUTE_REQUEST(_method, _path) \
	rtr.route(std::make_shared<HTTPRequest>(_method, _path), std::make_shared<HTTPResponse>(nullptr, on_data, on_end))
...
TEST_CASE("Test callbacks")
{
	...
	auto on_data = [&](std::shared_ptr<HTTPResponse> res, std::vector<char> data) {};
	auto on_end = [&](std::shared_ptr<HTTPResponse> res) {
		last_code = res->code;
	};
